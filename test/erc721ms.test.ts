...
import { ERC721MS } from '../typechain-types';

const { keccak256, getAddress } = ethers.utils;

chai.use(chaiAsPromised);
describe('ERC721MS', function () {
    let contract: ERC721MS;
    let readonlyContract: ERC721MS;
    let owner: SignerWithAddress;
    let readonly: SignerWithAddress;
    let chainId: number;
  
    const getCosignSignature = async (
      contractInstance: ERC721MS,
      cosigner: SignerWithAddress,
      minter: string,
      timestamp: number,
      qty: number,
    ) => {
      const nonce = await contractInstance.tokensOfOwner(minter);
    ...
    beforeEach(async () => {
      ...
      const erc20k = await ERC20K.deploy('ERC20', 'ETST', 1000000);

      const ERC721M = await ethers.getContractFactory(ContractDetails.ERC721MS.name);
